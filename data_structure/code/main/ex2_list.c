...
static void print_list (const List *list) {
    ...
    while (1) {
        data = list_data(node);
        fprintf(stdout, "list.node[%03d]=%c, %p -> %p \n", i, *data, node, node->next);//cambie %03d por %c
...
int main (int argc, char **argv) { 
    ...
    char *data, i; //cambiar tipo de dato de int a char
    ...
    node = list_head(&list);
    for (i = 65; i < 91; i++) { //agregar 65 a 90 del codigo ASCII A la Z

        if ((data = (char *)malloc(sizeof(char))) == NULL)//cambiar tipo de dato de int a char
            return 1;
